...
def get_should_long(prices: np.array) -> Tuple[float, float]:
    ...
    if(len(prices) > 21):
        ...
        prev_moving_avg_slow = pd.Series(prices[:-2]).rolling(slow).mean()
        prev_moving_avg_fast = pd.Series(prices[:-2]).rolling(fast).mean()
        ...
        return {
            ...
            "bollinger_band_difference":  (prices[-1] - bollinger_high),
            "ema_difference": (moving_avg_fast.iloc[-1] - moving_avg_slow.iloc[-1])/prices[-1],
            "prev_ema_difference": (prev_moving_avg_fast.iloc[-1] - prev_moving_avg_slow.iloc[-1])/prices[-2],
        }
    return None
